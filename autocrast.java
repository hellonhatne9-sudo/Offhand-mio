package ncore.features.modules.combat;
...
import ncore.core.InputBlocker;
import ncore.core.Managers;
import ncore.core.manager.client.AsyncManager;
import ncore.core.manager.client.ModuleManager;
import ncore.events.impl.EventFixVelocity;
import ncore.events.impl.EventKeyboardInput;
import ncore.events.impl.EventSync;
import ncore.events.impl.PacketEvent;
import ncore.events.impl.TotemPopEvent;
import ncore.features.modules.Module;
import ncore.injection.accesors.IInteractionManager;
import ncore.setting.Setting;
import ncore.setting.impl.Bind;
import ncore.setting.impl.SettingGroup;
import ncore.utility.player.InteractionUtility;
import ncore.utility.player.InventoryUtility;
import ncore.utility.player.MovementUtility;
import ncore.utility.player.PlayerUtility;
import ncore.utility.player.SearchInvResult;
import ncore.utility.render.Render2DEngine;
import ncore.utility.render.Render3DEngine;
...
public class AutoCart extends Module {
   ...
   private final Setting<Float> maxDistance = new Setting<>("Max Distance", 4.5F, 2.0F, 6.0F,
         v -> this.mode.is(AutoCart.Mode.Bow));
   private final Setting<Integer> startDelay = new Setting<>("Start Delay", 25, 0, 60,
         v -> this.mode.is(AutoCart.Mode.Bow));
   private final Setting<Bind> activeBind = new Setting<>("Active Bind", new Bind(-1, false, false),
         v -> this.mode.is(AutoCart.Mode.CrossBow));
   private final Setting<Integer> delay = new Setting<>("Delay", 25, 0, 100,
         v -> this.mode.is(AutoCart.Mode.Bow) || this.mode.is(AutoCart.Mode.CrossBow));
   private final Setting<Integer> cartAuraDelay = new Setting<>("Cart Aura Delay", 0, 0, 20,
         v -> this.isCartAuraEnabled());
   ...
   private final Setting<Boolean> cartAura = new Setting<>("Cart Aura", false,
         v -> this.mode.is(AutoCart.Mode.CrossBow));
   ...
   private final Setting<Boolean> cartAuraTarget = new Setting<>("Aura Target", true,
         v -> this.mode.is(AutoCart.Mode.CrossBow) && this.cartAura.getValue())
         .addToGroup(this.cartAuraTargets);
   private final Setting<Boolean> cartOtherPlayer = new Setting<>("Other Player", false,
         v -> this.mode.is(AutoCart.Mode.CrossBow) && this.cartAura.getValue())
         .addToGroup(this.cartAuraTargets);
   ...
   private void handleRefill() {
      if (this.reFill.is(AutoCart.ReFillMode.None)) {
         this.clearRefillState();
      } else if (!this.scheduledRefillSwap && mc.currentScreen == null
         ...
         if (this.mode.is(AutoCart.Mode.CrossBow) && this.cartAura.getValue() && !this.cartAuraExecuting) {
            if (popTarget != null && popTarget != mc.player) {
               ...
               if (this.cartOtherPlayer.getValue()) {
                  isOtherPlayer = !Managers.FRIEND.isFriend(popTarget)
   ...
   private void executeCartAura(PlayerEntity target) {
      ...
      Managers.ASYNC.run(() -> {
         try {
            ...
            if (fullNullCheck() || this.isDisabled() || !this.mode.is(AutoCart.Mode.CrossBow)
                  || !this.cartAura.getValue()) {
               return;
            }
   ...
   private boolean isSameAuraTarget(Entity auraTarget, PlayerEntity player) {
      return !(auraTarget instanceof PlayerEntity auraPlayer)
            ? false
            : auraPlayer == player || auraPlayer.getId() == player.getId()
                  || auraPlayer.getUuid().equals(player.getUuid());
   }
            ...
            if (targetPos != null) {
               ...
               if (railExists || this.findRailInHotBar().found()) {
                  ...
                  if (!(distSq > maxDistSq) && !(distSq < safeDistSq)) {
                     Managers.ASYNC.run(() -> this.executeBowPlacement(targetPos),
                           this.startDelay.getValue().intValue());
                  }
               }
            }
         }
   ...
   private AutoCart.CartAuraPlan createCartAuraPlan(PlayerEntity target) {
      ...
      if (crossbowResult.found() && cartResult.found()) {
         ...
         return !railExists && !railResult.found()
               ? null
               : new AutoCart.CartAuraPlan(basePos, crossbowResult, railResult, cartResult, flintResult, hasFlame,
                     railExists);
      } else {
         return null;
      }
                  ...
                  if (!(PlayerUtility.squaredDistanceFromEyes(hitPos.toCenterPos()) > 20.25F)) {
                     ...
                     Managers.ASYNC.run(() -> {
                        ...
                        if (!hasFlame && finalFireBlockPos != null) {
                           mc.executeSync(() -> {
                              Vec3d fireVec = new Vec3d(finalFireBlockPos.getX() + 0.5, finalFireBlockPos.getY() + 1.0,
                                    finalFireBlockPos.getZ() + 0.5);
                              this.applyRotation(InteractionUtility.calculateAngle(fireVec));
                              this.selectHotbarLegit(flintResult.slot());
                              BlockHitResult fireHit = new BlockHitResult(fireVec, Direction.UP, finalFireBlockPos,
                                    false);
                              this.interactWithBlock(fireHit);
                           });
                           AsyncManager.sleep(delayMs);
                        ...
                        mc.executeSync(() -> {
                           Vec3d minecartCenter = new Vec3d(basePos.getX() + 0.5, basePos.getY() + 1.5,
                                 basePos.getZ() + 0.5);
                           float[] shootAngle = InteractionUtility.calculateAngle(minecartCenter);
                           this.applyRotation(shootAngle);
   ...
   private float[] getAutoCartMoveFixRotation() {
      ...
      return this.isOn() && !fullNullCheck() && !this.changeLook.getValue() && this.rotating && rotation != null
            && !mc.player.isRiding() ? rotation : null;
   }
   ...
   private void interactWithItem() {
      if (mc.player != null && mc.interactionManager != null) {
         this.runWithInteractionRotation(this.silentRotation,
   ...
   private void placeRailOn(BlockPos basePos) {
      if (mc.world != null && mc.player != null && mc.interactionManager != null) {
         BlockHitResult hitResult = new BlockHitResult(
               new Vec3d(basePos.getX() + 0.5, basePos.up().getY(), basePos.getZ() + 0.5), Direction.UP, basePos,
               false);
   ...
   private void placeMinecartOn(BlockPos basePos) {
      if (mc.world != null && mc.player != null && mc.interactionManager != null) {
         BlockHitResult hitResult = new BlockHitResult(
               new Vec3d(basePos.getX() + 0.5, basePos.up().getY() + 0.125, basePos.getZ() + 0.5), Direction.UP,
               basePos.up(), false);
         this.interactWithBlock(hitResult);
      }
   }
         ...
         for (int i = 0; i < 300; i++) {
            ...
            for (Entity ent : mc.world.getEntities()) {
               if (!(ent instanceof ArrowEntity)
                     ...
                     && ent.getBoundingBox()
                           .intersects(new Box(x - 0.3, y - 0.3, z - 0.3, x + 0.3, y + 0.3, z + 0.3))) {
                  return null;
               }
            }

            Vec3d pos = new Vec3d(x, y, z);
            BlockHitResult bhr = mc.world
                  .raycast(new RaycastContext(lastPos, pos, ShapeType.OUTLINE, FluidHandling.NONE, mc.player));
                     ...
                     if (aboveState.isReplaceable() || this.isRailBlock(aboveState.getBlock())) {
                        ...
                        if (!(PlayerUtility.squaredDistanceFromEyes(surfacePos) > 20.25F)) {
                           BlockHitResult blockCheck = mc.world
                                 .raycast(new RaycastContext(playerEyes, surfacePos, ShapeType.COLLIDER,
                                       FluidHandling.NONE, mc.player));
                           if ((blockCheck == null || blockCheck.getType() != Type.BLOCK
                                 || blockCheck.getBlockPos().equals(bp))
                              ...
                              BlockHitResult damageCheck = mc.world
                                    .raycast(new RaycastContext(cartCenter, targetFeet, ShapeType.COLLIDER,
                                          FluidHandling.NONE, mc.player));
   ...
   private SearchInvResult findRailInHotBar() {
      return InventoryUtility.findItemInHotBar(Items.RAIL, Items.ACTIVATOR_RAIL, Items.DETECTOR_RAIL,
            Items.POWERED_RAIL);
   }
   ...
   private boolean isCrossbowDebugCandidate(ItemStack stack) {
      return !this.isUnloadedCrossbow(stack) ? false
            : this.hasFlameEnchant(stack) || InventoryUtility.findItemInHotBar(Items.FLINT_AND_STEEL).found();
   }
   ...
   private boolean isRailBlock(Block block) {
      return block == Blocks.RAIL || block == Blocks.POWERED_RAIL || block == Blocks.DETECTOR_RAIL
   ...
   private boolean hasFlameEnchant(ItemStack stack) {
      return mc.world == null
            ...
            : EnchantmentHelper.getLevel(
                  (RegistryEntry) mc.world.getRegistryManager().get(Enchantments.FLAME.getRegistryRef())
                        .getEntry(Enchantments.FLAME).get(),
                  stack) > 0;
   }
